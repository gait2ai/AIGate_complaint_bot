...
async def initialize_cache_manager(config: AppConfig) -> Optional[CacheManager]:
    ...
    try:
        ...
        APP_ROOT = Path(__file__).parent.absolute()
        cache_dir = APP_ROOT / cache_config.cache_dir
        ...
        # Pass the CacheModel object directly instead of converting to dict
        # This allows CacheManager to use attribute access internally
        cache_manager = CacheManager(
            cache_dir=cache_dir,
            config=cache_config,  # Pass Pydantic model directly
