...
def load_configuration() -> Dict[str, Any]:
    ...
    try:
        ...
        required_sections = ['institution', 'institution_bot_settings', 'ai_models', 'cache', 'prompts']
...
async def initialize_prompt_builder(config: Dict[str, Any]) -> PromptBuilder:
    ...
    try:
        ...
        template_file = templates.get('system_template_file', 'institution_system_prompt.txt')
...
async def initialize_institution_bot(
    ...
    try:
        ...
        logging.info("Initializing InstitutionBot")
        ...
        logging.info("InstitutionBot initialized successfully")
...
async def run_application():
    ...
    try:
        ...
        logging.info("=" * 60)
        logging.info("Institution Complaint Management Bot Starting Up")
        logging.info("=" * 60)
        
        # Define database path
        APP_ROOT = Path(__file__).parent.absolute()
        db_dir = APP_ROOT / "app" / "database"
        db_path = db_dir / "ins_data.db"
        
        # Initialize DatabaseManager
        logging.info("Step 1/5: Initializing DatabaseManager...")
        try:
            database_manager_instance = DatabaseManager(db_path=str(db_path))
            await asyncio.to_thread(database_manager_instance.connect)
            await asyncio.to_thread(database_manager_instance.create_tables)
        ...
        logging.info("Starting Institution Telegram Bot...")
        logging.info("=" * 60)
        
        # Run the bot application
        await institution_bot_instance.run()
