...
- UPDATED: Added critical_complaint_config parameter to PromptBuilder initialization
...
async def initialize_prompt_builder(config: AppConfig) -> PromptBuilder:
    """
    Initialize the PromptBuilder with the new three-template structure, analysis settings,
    and critical complaint configuration.
    ...
    - UPDATED: Now requires critical_complaint_config parameter for dynamic prompt generation
    ...
    try:
        ...
        analysis_settings = config.analysis_settings  # Get analysis settings from config
        ...
        # UPDATED INSTANTIATION LOGIC: Pass three template paths, analysis settings, and critical complaint config
        prompt_builder = PromptBuilder(
            ...
            analysis_settings=analysis_settings,     # Pass AnalysisSettingsModel
            critical_complaint_config=config.critical_complaint_config  # ADDED: Pass CriticalComplaintConfigModel
        )
        ...
        logging.error("Ensure all three template files exist, analysis_settings is properly configured, and critical_complaint_config is present")
        sys.exit(1)
...
async def initialize_email_service(config: AppConfig) -> EmailService:
    ...
    try:
        ...
        email_service = EmailService(email_config=config.email_config,
    institution_name=config.institution.name_en)
...
async def initialize_institution_bot(
    ...
    Args:
        ...
        cache_manager: CacheManager instance or None
        prompt_builder: PromptBuilder instance (now with three-template structure and critical complaint config)
    ...
    try:
        ...
        institution_bot = InstitutionBot(
            ...
            cache_manager=cache_manager,
            prompt_builder=prompt_builder,  # Now initialized with three-template structure and critical complaint config
        ...
        logging.info("InstitutionBot initialized successfully with AppConfig integration, refactored PromptBuilder with critical complaint config, v20+ persistence, and EmailService")
...
async def run_application():
    ...
    - UPDATED: PromptBuilder initialization for new three-template structure with analysis settings and critical complaint config
    ...
    5. PromptBuilder Initialization (UPDATED: using three templates + analysis settings + critical complaint config)
    ...
    10. Handler Registration (centralized)
    11. Polling Start
    """
    global ai_handler_instance, cache_manager_instance, institution_bot_instance, database_manager_instance, email_service_instance
    ...
    try:
        ...
        logging.info("UPDATED: PromptBuilder with Three-Template Structure + Analysis Settings + Critical Complaint Config")
        logging.info("=" * 80)
        ...
        # Step 5: Initialize PromptBuilder with THREE TEMPLATES + ANALYSIS SETTINGS + CRITICAL COMPLAINT CONFIG
        logging.info("Step 4/9: Initializing PromptBuilder with THREE-TEMPLATE structure, analysis settings, and critical complaint config...")
        ...
        logging.info("Step 7/9: Initializing InstitutionBot with complete AppConfig, refactored PromptBuilder with critical complaint config, v20+ persistence, and EmailService...")
        ...
        # Step 10: Register all handlers using centralized system
        logging.info("Step 9/9: Registering handlers via centralized system...")
        ...
        logging.info("PromptBuilder updated with three-template structure + analysis settings + critical complaint config.")
        logging.info("=" * 80)
