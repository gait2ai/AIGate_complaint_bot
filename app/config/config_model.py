...
class ComplaintData(BaseModel):
    ...
    age: int = Field(default=0, description="Age of the complainant")
    nationality: str = Field(default="", description="Nationality of the complainant")
    ...
    disability: str = Field(default="", description="Whether the complainant has a disability")
...
class ApiKeys(BaseSettings):
    ...
    moonshot_api_key: Optional[str] = Field(default=None, description="API Key for Moonshot AI (Kimi)")
...
class DataCollectionFields(BaseModel):
    ...
    age: bool = Field(default=True, description="Collect user's age")
    disability: bool = Field(default=True, description="Collect disability information")
    nationality: bool = Field(default=True, description="Collect user's nationality")
...
class CriticalComplaintConfigModel(BaseModel):
    ...
    keywords: List[str] = Field(default_factory=list, description="A list of keywords to programmatically flag a complaint as critical.")
...
class DirectModelProviderModel(BaseModel):
    """Model for a direct AI provider configuration like Moonshot."""
    provider: str = Field(..., description="The name of the provider (e.g., 'moonshot')")
    base_url: HttpUrl = Field(..., description="The base API URL for the direct provider")
    model: str = Field(..., description="The specific model name to use for the direct provider")
    max_tokens: int = Field(default=2048, description="Maximum tokens for the model's responses")
    temperature: float = Field(default=0.3, description="AI response temperature for this model")
    timeout: int = Field(default=30, description="API timeout in seconds for this provider")
...
class AiModelsModel(BaseModel):
    ...
    direct_model: Optional[DirectModelProviderModel] = Field(default=None, description="Configuration for a high-priority direct model provider.")
...
class SectorModel(BaseModel):
    """Model for a single sector with its description."""
    name: str = Field(..., description="The name of the sector.")
    description: str = Field(..., description="A brief description of what this sector covers.")
    
class SensitivityLevelModel(BaseModel):
    """Model for a single sensitivity level with its description."""
    name: str = Field(..., description="The name of the sensitivity level.")
    description: str = Field(..., description="A brief description of this level.")
...
class AnalysisSettingsModel(BaseModel):
    ...
    sectors: List[SectorModel] = Field(default_factory=list, description="List of sectors with their descriptions for analysis.")
    sensitivity_levels: List[SensitivityLevelModel] = Field(default_factory=list, description="List of sensitivity levels with their descriptions for analysis.")
    
class ApplicationMetaModel(BaseModel):
    """Model for application metadata and license key."""
    license_key: str = Field(..., description="A base64 encoded string containing developer and version information.")
...
class AppConfig(BaseModel):
    ...
    application_meta: ApplicationMetaModel = Field(..., description="Application metadata and author license key.")
