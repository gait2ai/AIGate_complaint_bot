...
class SmtpCredentials(BaseSettings):
    """
    Model for loading SMTP email server credentials from environment variables.
    
    Contains sensitive email authentication information that should never
    be stored in version control.
    """
    sender_email: EmailStr = Field(..., env='SMTP_EMAIL', description="SMTP sender email address")
    sender_password: str = Field(..., env='SMTP_PASSWORD', description="SMTP sender password")
...
class EmailConfigModel(BaseModel):
    ...
    This model parses non-sensitive email settings from YAML and automatically
    injects SMTP credentials from environment variables.
    """
    ...
    sender_email: str = Field(..., description="Sender email from YAML (will be overridden by credentials)")
    sender_password: str = Field(..., description="Sender password from YAML (will be overridden by credentials)")
    ...
    credentials: SmtpCredentials = Field(default_factory=SmtpCredentials, description="SMTP credentials from environment")
