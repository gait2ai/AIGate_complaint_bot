...
class LLMOrchestrator:
    ...
    the user's initial intent from their first message and identifying critical
    ...
    async def analyze_initial_message(
        ...
        Returns:
            ...
            - is_critical (bool): Flag indicating if the complaint requires immediate attention
        """
        self.logger.info(
        ...
        try:
            ...
            signal, response_text, is_critical = self._validate_and_extract_data(parsed_response)
            ...
            return signal, final_response_text, is_critical
            ...
            signal, response_text, is_critical = self._get_safe_default_response()
            final_default_text = response_text.format(
                institution_name=self.prompt_builder.institution_config.name
            )
            return signal, final_default_text, is_critical
    ...
    def _validate_and_extract_data(
        ...
        Returns:
            Tuple of (signal, response_text, is_critical)
        """
        # Extract basic fields
        signal = parsed_response.get("signal")
        response_text = parsed_response.get("response_text")
        is_critical = parsed_response.get("is_critical", False)
        ...
        # Validate is_critical flag
        if not isinstance(is_critical, bool):
            self.logger.warning(f"Invalid is_critical value received: {is_critical}. Defaulting to False.")
            is_critical = False
        
        self.logger.debug(f"Data validation completed for signal: {signal}, is_critical: {is_critical}")
        return signal, response_text, is_critical
    
    def _get_safe_default_response(self) -> Tuple[str, str, bool]:
        """
        Get a safe default response for error scenarios.
        
        Returns:
            Safe default tuple (signal, response_text, is_critical)
        """
        return (
            "GENERAL_INQUIRY",
