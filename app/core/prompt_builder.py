...
class PromptBuilder:
    ...
    def __init__(self, config_dir: Path, institution_config, prompts_config):
        """
    ...
    def _load_system_prompt_template(self) -> str:
        ...
        prompts_config.system_template_file (should be "system_prompt.txt").
        If loading fails, it falls back to a basic Institution-specific template.
        
        Returns:
            str: Institution system prompt template
        """
        # Step 1: Retrieve filename from configuration using Pydantic model
        configured_filename = getattr(self.prompts_config, 'system_template_file', None)
...
{language_instruction}"""
    ...
    async def generate_initial_interaction_prompt(
        ...
        current_date_time_text: Optional[str] = None
    ) -> str:
        """
        Generate a specialized system prompt for initial user message analysis performed by LLMOrchestrator.
        
        This method creates a comprehensive system prompt specifically tailored for the "initial user message 
        analysis" task by populating the institution_system_prompt.txt template with contextual information
        including user details, critical complaint criteria, and timestamp information.
        ...
        Args:
            ...
            critical_complaint_criteria_text: Optional text containing critical complaint identification criteria.
                                             If None or empty, a default placeholder will be used.
            current_date_time_text: Optional timestamp information for the interaction.
                                   If None or empty, a default placeholder will be used.
        
        Returns:
            str: Complete system prompt ready for LLM consumption, formatted specifically for initial
                 user message analysis with all template placeholders populated.
        ...
        try:
            ...
            # Retrieve institution name from Pydantic model
            institution_name = getattr(self.institution_config, 'name', 'Institution')
            
            # Prepare context data with critical complaint criteria
            context_data = critical_complaint_criteria_text
            if not context_data or not context_data.strip():
                context_data = "No specific critical complaint criteria provided for this interaction."
            
            # Prepare current date/time information
            current_date_time = current_date_time_text
            if not current_date_time or not current_date_time.strip():
                current_date_time = "Timestamp not available for this interaction."
            ...
            logger.debug(f"Template variables prepared: institution_name={institution_name}, "
                        ...
                        f"context_data_length={len(context_data)}, "
                        f"current_date_time={current_date_time}, "
                        f"user_input_length={len(user_input_text)}")
            
            # Format the system prompt template with prepared variables
            formatted_prompt = self.system_prompt_template.format(**template_variables)
            ...
            error_msg = f"Missing template placeholder in system prompt template: {e}"
    ...
    async def build_institution_task_prompt(self,
                                    ...
                                    user_language_code: str = 'ar',
                                    additional_template_vars: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        Args:
            ...
            additional_template_vars: Optional additional template variables for future extensibility
                                    (e.g., user_first_name, current_date_time for enhanced templates)
        ...
        try:
            ...
            # Get institution name from Pydantic model
            institution_name = getattr(self.institution_config, 'name', 'Institution')
            ...
            # Add additional template variables if provided (for future extensibility)
            if additional_template_vars:
                # Only add variables that are not already set and exist as placeholders in template
                for key, value in additional_template_vars.items():
                    placeholder = f"{{{key}}}"
                    if placeholder in self.system_prompt_template and key not in template_variables:
                        template_variables[key] = value
                        logger.debug(f"Added additional template variable: {key}")
            
            # Format the system prompt with all placeholders
            try:
                formatted_prompt = self.system_prompt_template.format(**template_variables)
            except KeyError as e:
                # Handle missing placeholders gracefully by providing default values
                missing_placeholder = str(e).strip("'")
                logger.warning(f"Missing placeholder {missing_placeholder} in template, using default value")
                
                # Add default values for common missing placeholders
                default_values = {
                    'user_first_name': 'User',
                    'current_date_time': 'Timestamp not available',
                    'context_data': context  # Use formatted context as fallback
                }
                
                if missing_placeholder in default_values:
                    template_variables[missing_placeholder] = default_values[missing_placeholder]
                    formatted_prompt = self.system_prompt_template.format(**template_variables)
                else:
                    raise  # Re-raise if we don't have a default value
            
            # Apply prompt optimization if enabled (check if attribute exists in Pydantic model)
            prompt_optimization = getattr(self.prompts_config, 'prompt_optimization', True)
...
Maintain professionalism, empathy, and follow proper complaint handling protocols."""
    ...
    async def get_template_variables(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'template_file': str(self.config_dir / getattr(self.prompts_config, 'system_template_file', 'system_prompt.txt')),
            'default_language': 'ar',
            'available_output_formats': list(self.default_output_formats.keys()),
            'institution_protocol_info': getattr(self.prompts_config, 'institution_protocol_info', {})
        }
