...
Please replace with actual template content."""
    ...
    def generate_final_analysis_prompt(self, complaint_text: str) -> str:
        ...
        data including allowed categories, category guidance, and sensitivity levels.
        ...
        try:
            ...
            # Process critical keywords from institution configuration
            critical_keywords = getattr(
                getattr(self.institution_config, 'critical_complaint_config', None),
                'keywords',
                []
            )
            
            if not critical_keywords:
                logger.warning("No critical keywords found in institution configuration")
