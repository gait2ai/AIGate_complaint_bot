...
Please replace with actual template content."""
    ...
    def _log_final_prompt_content(self, prompt_content: str, prompt_type: str, metadata: Dict[str, Any] = None):
        """
        Log the final prompt content for inspection purposes.
        
        Args:
            prompt_content: The final formatted prompt content
            prompt_type: Type of prompt (e.g., 'initial_analysis', 'final_analysis', 'input_validation')
            metadata: Additional metadata about the prompt generation
        """
        try:
            ...
            # Log at INFO level for normal operation visibility
            logger.info(f"=== FINAL PROMPT INSPECTION ({prompt_type.upper()}) ===")
            logger.info(f"Timestamp: {inspection_data['timestamp']}")
            logger.info(f"Prompt Type: {inspection_data['prompt_type']}")
            logger.info(f"Prompt Length: {inspection_data['prompt_length']} characters")
            
            # Log metadata if provided
            if metadata:
                logger.info(f"Metadata: {json.dumps(metadata, indent=2, ensure_ascii=False)}")
            
            # Log the full prompt content with clear delimiters
            logger.info("=== PROMPT CONTENT START ===")
            logger.info(prompt_content)
            logger.info("=== PROMPT CONTENT END ===")
            
            # Log a summary line
            logger.info(f"Final prompt for {prompt_type} ready for AI model consumption")
            
        except Exception as e:
            logger.error(f"Error logging final prompt content: {e}")
    ...
    async def generate_initial_interaction_prompt(
        ...
        try:
            ...
            # Prepare metadata for inspection logging
            inspection_metadata = {
                'user_first_name': user_first_name,
                'institution_name': institution_name,
                'detected_language': language,
                'user_input_length': len(user_input_text),
                'template_variables': {k: v if k != 'user_message' else f"[{len(v)} chars]" for k, v in template_variables.items()},
                'critical_complaint_criteria_provided': critical_complaint_criteria_text is not None,
                'current_date_time': current_date_time
            }
            
            # Log the final prompt content for inspection
            self._log_final_prompt_content(formatted_prompt, 'initial_analysis', inspection_metadata)
    ...
    def generate_final_analysis_prompt(self, complaint_text: str) -> str:
        ...
        try:
            ...
            # Prepare metadata for inspection logging
            inspection_metadata = {
                'complaint_text_length': len(complaint_text),
                'categories_count': len(complaint_categories) if complaint_categories else 0,
                'sensitivity_levels_count': len(sensitivity_levels) if sensitivity_levels else 0,
                'critical_keywords_count': len(critical_keywords) if critical_keywords else 0,
                'template_variables': {k: v if k != 'complaint_text' else f"[{len(v)} chars]" for k, v in template_variables.items()},
                'configuration_status': {
                    'categories_configured': len(complaint_categories) > 0,
                    'sensitivity_levels_configured': len(sensitivity_levels) > 0,
                    'critical_keywords_configured': len(critical_keywords) > 0
                }
            }
            
            # Log the final prompt content for inspection
            self._log_final_prompt_content(formatted_prompt, 'final_analysis', inspection_metadata)
    ...
    def generate_input_validation_prompt(self, question_asked: str, user_answer: str) -> str:
        ...
        try:
            ...
            # Prepare metadata for inspection logging
            inspection_metadata = {
                'question_length': len(question_asked),
                'answer_length': len(user_answer),
                'template_variables': {k: f"[{len(v)} chars]" for k, v in template_variables.items()},
                'validation_context': {
                    'question_preview': question_asked[:50] + "..." if len(question_asked) > 50 else question_asked,
                    'answer_preview': user_answer[:50] + "..." if len(user_answer) > 50 else user_answer
                }
            }
            
            # Log the final prompt content for inspection
            self._log_final_prompt_content(formatted_prompt, 'input_validation', inspection_metadata)
