...
class CacheManager:
    ...
    def __init__(
        self,
        config,  # CacheModel - Pydantic model instance
        loop: Optional[asyncio.AbstractEventLoop] = None
    ):
        """
        Initialize cache manager with Pydantic CacheModel configuration.
        
        Args:
            config: CacheModel instance containing cache configuration
            loop: Event loop instance for async operations (will attempt to get current loop if None)
        """
        # Store the CacheModel instance directly
        self.config = config
        
        # Extract configuration values using attribute access
        ...
        # Cache category configurations - use config.categories if available
        ...
        # Update category configs from CacheModel categories if available
        if hasattr(config, 'categories') and config.categories:
            self._category_configs.update(config.categories)
    ...
    def _start_cleanup_task(self):
        """Start background cleanup task with configurable interval."""
        # Get cleanup interval from config using attribute access
