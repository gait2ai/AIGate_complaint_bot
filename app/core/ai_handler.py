        ...
        try:
            async with self.session.post(
                ...
                if r.status == 200:
                    ...
                    return True, txt, {"response_time": time.time() - start}
                else:
...
class AIHandler:
    ...
    def _initialize_models(self):
        ...
        # Add the direct model (e.g., Kimi) first if it exists
        direct_model_config = self.config.get('direct_model', {})
        if direct_model_config and direct_model_config.get('model'):
            provider = direct_model_config.get('provider', 'moonshot')
            model_name = direct_model_config.get('model')
            models_list.append((provider, model_name))
            logger.info(f"Direct model configured with top priority: {model_name} ({provider})")
    ...
    def _initialize_providers(self):
        ...
        # Initialize Moonshot provider if configured
        direct_model_config = self.config.get('direct_model', {})
        if direct_model_config and direct_model_config.get('provider') == 'moonshot':
            self.providers['moonshot'] = MoonshotProvider(direct_model_config, self.session)
    ...
    async def generate_response(self, user_message: str, system_prompt: str, 
        ...
        Now iterates through available models upon failure.
        ...
        # ---> التحسين الرئيسي هنا: حلقة على كل النماذج المتاحة <---
        # First, create a list of models to try, respecting their order in the config
        models_to_try = []
        for model_info in self.models:
            if self.providers.get(model_info.provider) and self.providers[model_info.provider].is_available():
                models_to_try.append(model_info)

        for model in models_to_try:
            # Check model status (e.g., rate-limited, too many failures) before attempting
            if model.status != ModelStatus.AVAILABLE:
                logger.debug(f"Skipping model {model.name} due to status: {model.status.value}")
                continue

            logger.info(f"Attempting request with model: {model.name} ({model.provider})")
            
            provider = self.providers[model.provider]
            payload = provider.build_payload(user_message, enhanced_prompt, model.name)
            
            # A single attempt for each model in the fallback chain
            success, response_text, metadata = await provider.make_request(payload, model)
            
            request_time = metadata.get('response_time', 0)
            self._update_model_metrics(model, success, request_time, None if success else response_text)
            ...
            if success:
                ...
                if is_valid:
                    ...
                    last_error = f"Response from {model.name} failed validation: {validation_error}"
                    logger.warning(last_error)
            else:
                last_error = f"Request to {model.name} failed: {response_text}"
                logger.warning(last_error)
                error_type = "rate_limit" if "rate limit" in response_text.lower() else "api_error"
                self.metrics.errors_by_type[error_type] = self.metrics.errors_by_type.get(error_type, 0) + 1
                
                # If we failed, we continue to the next model in the list
                continue
