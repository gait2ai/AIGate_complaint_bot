...
Updated to use Pydantic models for type-safe configuration management.
"""
...
class HuggingFaceDirectProviderModel(BaseModel):
    """Pydantic model for Hugging Face provider configuration."""
    api_key_env_var: str = "HF_API_TOKEN"
    primary_model_hf: Optional[str] = None
    fallback_models_hf: Optional[List[str]] = None
    base_url_hf: str = "https://api-inference.huggingface.co/models/"
    hf_timeout: int = 45
...
class AiModelsModel(BaseModel):
    """Pydantic model for AI models configuration."""
    primary_model: str
    ...
    # --- هذه هي الحقول التي كانت مفقودة أو في غير مكانها ---
    direct_fallback_enabled: bool = True
    stream_responses: bool = False
    
    # --- هذه الحقول يمكن أن تكون اختيارية ---
    ...
    huggingface_direct_provider: Optional[HuggingFaceDirectProviderModel] = None
...
class OpenRouterProvider(AIProvider):
    ...
    def __init__(self, config: AiModelsModel, session: aiohttp.ClientSession):
        """
        Initialize OpenRouter provider with Pydantic configuration.
        
        Args:
            config: AiModelsModel instance containing configuration
...
class HuggingFaceProvider(AIProvider):
    ...
    def __init__(self, config: AiModelsModel, session: aiohttp.ClientSession):
        """
        Initialize Hugging Face provider with Pydantic configuration.
        ...
        hf_config = config.huggingface_direct_provider
        if hf_config:
            self.api_key = os.getenv(hf_config.api_key_env_var)
            self.base_url_template = hf_config.base_url_hf + "{}"
            self.timeout = hf_config.hf_timeout
            self.max_new_tokens = hf_config.hf_max_new_tokens
            self.temperature = hf_config.hf_temperature or config.temperature
        else:
            # Fallback to defaults if no HF config provided
...
class AIHandler:
    ...
    def __init__(self, config: AiModelsModel, cache_manager=None):
        """
        Initialize the AI Handler with type-safe Pydantic configuration.
        
        Args:
            config: AiModelsModel instance containing AI configuration
        ...
        # Configuration parameters using Pydantic attributes
        ...
        # Validation settings using Pydantic attributes
        self.min_response_length = config.min_response_length
        self.max_response_length = config.max_response_length
        self.preserve_markdown = config.preserve_markdown
        
        # Fallback responses from configuration
        self.fallback_responses = config.fallback_responses or [
    ...
    def _initialize_models(self):
        """Initialize model list from Pydantic configuration."""
        models_list = []
        
        # OpenRouter models using Pydantic attributes
        ...
        # Hugging Face models (if enabled) using Pydantic attributes
        if self.config.direct_fallback_enabled and self.config.huggingface_direct_provider:
            hf_config = self.config.huggingface_direct_provider
            hf_primary = hf_config.primary_model_hf
            hf_fallbacks = getattr(hf_config, 'fallback_models_hf', []) or []
        ...
        if not models_list:
            ...
            logger.warning(f"No models configured in Pydantic config, using embedded defaults")
    ...
    def _initialize_providers(self):
        """Initialize AI providers with Pydantic configuration."""
        self.providers = {}
        
        # Initialize OpenRouter provider with Pydantic config
        self.providers['openrouter'] = OpenRouterProvider(self.config, self.session)
        
        # Initialize Hugging Face provider with Pydantic config
        self.providers['huggingface'] = HuggingFaceProvider(self.config, self.session)
    ...
    def _validate_response(self, response_text: str) -> Tuple[bool, Optional[str]]:
        ...
        # Check length constraints using Pydantic attributes
    ...
    def _format_response(self, response_text: str) -> str:
        ...
        # Remove any markdown artifacts if not needed (using Pydantic attribute)
