...
class InstitutionBot:
        ...
        try:
            ...
            if result:
                return {
                    ...
                    'sex': result[1],
                    'phone': result[2],
                    'residence_status': result[3],
                    'governorate': result[4],
                    'directorate': result[5],
                    'village_area': result[6]
                }
            ...
            if existing:
                ...
                self.db_manager.execute_query(
                    """UPDATE beneficiaries SET 
                       name = ?, sex = ?, phone = ?, residence_status = ?, 
                       governorate = ?, directorate = ?, village_area = ?, updated_at = ?
                       WHERE user_telegram_id = ?""",
                    (data.name, data.sex, data.phone, data.residence_status,
                     data.governorate, data.directorate, data.village, timestamp, data.user_id)
                )
            else:
                ...
                self.db_manager.execute_query(
                    """INSERT INTO beneficiaries 
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (data.user_id, data.name, data.sex, data.phone, data.residence_status,
                     data.governorate, data.directorate, data.village, timestamp, timestamp)
                )
    ...
    async def _log_complaint(self, data: ComplaintData) -> bool:
        ...
        try:
            # First, ensure beneficiary profile is saved/updated
            await self._save_beneficiary_profile(data)
            ...
            if not beneficiary_result:
                self.logger.error(f"Could not find beneficiary ID for user {data.user_id}")
                return False
            
            beneficiary_id = beneficiary_result[0]
            
            # Get classification using updated method with consistent keys
            ...
            # Insert complaint into database with updated column names
            timestamp = datetime.now().isoformat()
