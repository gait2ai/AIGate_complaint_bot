...
# Import the Pydantic configuration model
...
class InstitutionBot:
    ...
    # New AI-Driven Initial Message Analysis
    ...
    async def analyze_first_contact_message(
        ...
        try:
            ...
            current_date_time_text = self._get_current_local_timestamp()
            ...
            signal, llm_response_text, user_facing_summary = await self.llm_orchestrator.analyze_initial_message(
                ...
                current_date_time_text=current_date_time_text
            )
    ...
    async def _save_beneficiary_profile(self, data: ComplaintData) -> bool:
        ...
        try:
            ...
            if not profile_data['name'] or profile_data['name'] == anonymous_user_name:
                # Fetch the user's first_name from the initial beneficiary record
                try:
                    result = self.db_manager.fetch_one(
    ...
    # Critical Complaint Email Method
    
    async def _send_critical_complaint_email(self, data: ComplaintData) -> bool:
        """
        Send email notification for critical complaints using centralized configuration.
        ...
        try:
            ...
            # Get email configuration using attribute access
            smtp_server = self.config.email.smtp_server
            smtp_port = self.config.email.smtp_port
            sender_email = self.config.email.sender_email
            sender_password = self.config.email.sender_password
            
            # Get critical complaint notification email using attribute access
            notification_email = self.config.critical_complaint_config.notification_email
            
            if not all([smtp_server, sender_email, sender_password, notification_email]):
                self.logger.error("Incomplete email configuration for critical complaint notification")
                return False
            ...
            msg['Subject'] = f"CRITICAL COMPLAINT - {self.config.institution.name_en}"
            
            # Email body
            body = f"""
CRITICAL COMPLAINT RECEIVED

User ID: {data.user_id}
Name: {data.name or 'Not provided'}
Phone: {data.phone or 'Not provided'}
Location: {data.governorate or 'Not provided'}
...
Submitted at: {self._get_current_local_timestamp()}

Please take immediate action to address this critical complaint.
            """
    ...
    # Implemented Complaint Reminder Methods
    ...
    async def get_user_previous_complaints_summary(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        """
        try:
            # Query to join complaints with beneficiaries table
            query = """
            ...
            complaints = self.db_manager.fetch_all(query, (user_id,))
    ...
    async def log_complaint_reminder_note(
        ...
        try:
            ...
            success = self.db_manager.add_complaint_note(
    ...
    async def get_complaint_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_result = self.db_manager.fetch_one("SELECT COUNT(*) FROM complaints")
            if total_result:
                stats['total_complaints'] = total_result[0]
            
            # Get critical complaints count
            critical_result = self.db_manager.fetch_one("SELECT COUNT(*) FROM complaints WHERE is_critical = 1")
            if critical_result:
                stats['critical_complaints'] = critical_result[0]
            
            # Get status counts
            status_results = self.db_manager.fetch_all("SELECT status, COUNT(*) FROM complaints GROUP BY status")
