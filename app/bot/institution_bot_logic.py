...
class InstitutionBot:
    ...
    async def export_data_to_csv(self, data_type: str) -> Optional[Tuple[str, bytes]]:
        """
        Export data of specified type to CSV format.
        
        Args:
            data_type: Type of data to export ('complaints', 'beneficiaries', or 'notes')
            
        Returns:
            Optional[Tuple[str, bytes]]: Tuple containing filename and file bytes, or None if failed
        """
        try:
            # Define data type specific parameters
            if data_type == 'complaints':
                headers = [
                    'ID', 'Reference ID', 'Beneficiary Name', 'Beneficiary Phone', 'Governorate',
                    'Directorate', 'Village/Area', 'Original Complaint Text', 'English Summary',
                    'Complaint Type', 'Complaint Category', 'Sensitivity Level', 'Is Critical',
                ...
                db_method = 'get_complaints_export_data'
            elif data_type == 'beneficiaries':
                headers = [
                    'ID', 'Telegram ID', 'Name', 'Sex', 'Phone', 'Residence Status',
                    'Governorate', 'Directorate', 'Village/Area', 'Last Seen At',
                    'Created At', 'Updated At'
                ]
                db_method = 'get_beneficiaries_export_data'
            elif data_type == 'notes':
                headers = [
                    'ID', 'Complaint ID', 'Complaint Reference ID', 'Note Text',
                    'Created By', 'Created At'
                ]
                db_method = 'get_notes_export_data'
            else:
                self.logger.error(f"Invalid data type for export: {data_type}")
                return None
            
            # Fetch data from database (async)
            db_data = await asyncio.to_thread(
                getattr(self.db_manager, db_method)
            )
            
            if not db_data:
                self.logger.info(f"No data available for export type: {data_type}")
                return None
            ...
            # Get CSV content and encode with BOM for Excel compatibility
            ...
            self.logger.info(f"Successfully generated {data_type} export with {len(db_data)} records")
            return filename, file_bytes
            
        except Exception as e:
            self.logger.error(f"Error exporting {data_type} data: {e}")
            return None
