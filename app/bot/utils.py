...
def get_message(message_key: str, bot_instance: 'InstitutionBot', is_arabic_reply: bool, **kwargs) -> str:
    ...
    try:
        ...
        if hasattr(bot_instance, 'config') and bot_instance.config:
            ...
            message_template = ui_messages.get(message_key, None)
...
def validate_phone_number(phone: str, bot_instance: 'InstitutionBot') -> bool:
    """
    Validate phone number format based on patterns from config.yaml.
...
def get_final_submission_inline_keyboard(bot_instance: 'InstitutionBot', is_arabic: bool) -> InlineKeyboardMarkup:
    """
    ...
    keyboard = [
        [InlineKeyboardButton(
            ...
            callback_data="final_submission:confirm"
        )],
        [InlineKeyboardButton(
            get_message('btn_cancel_submission', bot_instance, is_arabic),
            callback_data="final_submission:cancel"
        )]
    ]
    return InlineKeyboardMarkup(keyboard)