...
uses explicit state management to prevent conversation hijacking.
...
async def prompt_enter_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Now implements stateful behavior by setting the conversation state to prevent
    conversation hijacking when the user responds with their suggestion text.
    ...
    try:
        ...
        # Set the conversation state to COLLECTING_SUGGESTION_TEXT
        # This prevents the main conversation handler from treating the user's
        # next text message as a new conversation start
        set_conversation_state(context, states.COLLECTING_SUGGESTION_TEXT)
        logger.info(f"Set conversation state to COLLECTING_SUGGESTION_TEXT for user {user.id}")
...
async def process_suggestion_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Clears the conversation state after successful processing since the next
    expected user action is a callback query (confirm/cancel button press).
    ...
    try:
        ...
        if len(suggestion_text) < min_length:
            ...
            # Keep the state as COLLECTING_SUGGESTION_TEXT to continue waiting for valid input
            return states.COLLECTING_SUGGESTION_TEXT
        ...
        # Clear the conversation state since we're moving to callback-based interaction
        # The next expected user action is a CallbackQuery (confirm/cancel button press)
        clear_conversation_state(context)
        logger.info(f"Cleared conversation state for user {user.id} after successful text processing")
        ...
        # Clear conversation state and clean up on error
        clear_conversation_state(context)
        await cleanup_conversation_state(update, context, reason="error_in_suggestion_processing")
        return ConversationHandler.END
...
async def handle_suggestion_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await cleanup_conversation_state(update, context, reason="suggestion_flow_completed")
        return ConversationHandler.END
        ...
        await cleanup_conversation_state(update, context, reason="error_in_suggestion_confirmation")
        return ConversationHandler.END