...
async def handle_initial_text_message(
    ...
    """
    if current_state and current_state != states.SELECTING_INITIAL_ACTION:
        logger.debug(f"نص تم تجاوزه أثناء الحالة النشطة: {current_state}")
        return None  # أو ConversationHandler.END إذا لزم الأمر
    try:
        ...
        if context.user_data.get('conversation_state') is not None:
            logger.debug("Active conversation detected, ignoring initial text")
            return None
        ...
        if signal == "GREETING_START":
            ...
            context.user_data['conversation_state'] = states.ASK_NEW_OR_REMINDER
            logger.debug(f"Explicitly setting state to ASK_NEW_OR_REMINDER for user {user.id}")
