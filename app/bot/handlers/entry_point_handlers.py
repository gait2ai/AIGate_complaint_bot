...
- Added proper placeholder formatting for AI-generated responses
"""
...
async def handle_initial_text_message(
    ...
    try:
        ...
        # Create dynamic language-specific institution name
        institution_name = (
            bot_instance.config.institution.name_ar if is_arabic 
            else bot_instance.config.institution.name_en
        )

        if signal == "GREETING_START":
            # Format the LLM response with placeholders
            formatted_response = llm_response_text.format(
                user_first_name=user.first_name,
            ...
            if user_facing_summary:
                # Format the user facing summary with placeholders
                formatted_summary = user_facing_summary.format(
                    user_first_name=user.first_name,
                    institution_name=institution_name
                )
