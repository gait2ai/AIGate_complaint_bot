...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Handles the /about command to display information about the bot's developer.
    
    This command retrieves the author information stored securely in the bot instance
    during initialization and formats it for display.
    """
    bot_instance: InstitutionBot = context.bot_data['bot_instance']
    is_arabic = get_user_preferred_language_is_arabic(update, bot_instance)
    
    try:
        # The author_info dictionary should have been loaded and verified at startup.
        if bot_instance.author_info:
            info = bot_instance.author_info
            
            # Format the message in Markdown for better presentation
            message_text = (
                f"*{bot_instance.config.institution.name_en} - Complaint Management Bot*\n\n"
                f"This bot was developed by: *{info.get('author', 'N/A')}*\n\n"
                f"__Contact the Developer:__\n"
                f"📧 Email: `{info.get('contact', 'Not available')}`\n"
                f"📞 Phone: `{info.get('phone', 'Not available')}`\n\n"
                f"_Version: {info.get('version', '1.0')}_"
            )
            
            await update.message.reply_text(message_text, parse_mode='Markdown')
            logger.info(f"/about command processed for user {update.effective_user.id}")
        else:
            # This case should ideally never be reached if the startup integrity check works.
            fallback_msg = "Developer information is currently unavailable."
            await update.message.reply_text(fallback_msg)
            logger.warning("Attempted to use /about command, but author_info was not available.")

    except Exception as e:
        logger.error(f"Error in about_command: {e}")
        await update.message.reply_text("An error occurred while retrieving developer information.")
...
def register_common_commands(application) -> None:
    ...
    try:
        ...
        # Register /about command handler
        application.add_handler(CommandHandler("about", about_command))
        logger.info("Registered /about command handler")
