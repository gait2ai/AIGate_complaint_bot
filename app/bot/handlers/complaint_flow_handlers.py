...
def _get_next_step_after_phone(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after phone collection.
    Returns the name of the next collect function to call.
    """
    config = bot_instance.config.application_settings.data_collection_fields
    
    # Check optional fields in order: email -> residence -> department -> position -> complaint_type
    if getattr(config, 'email', False):
        return 'email'
    elif getattr(config, 'residence_status', False):
...
def _get_next_step_after_email(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after email collection.
    """
    config = bot_instance.config.application_settings.data_collection_fields
    
    if getattr(config, 'residence_status', False):
        return 'residence_status'
    elif getattr(config, 'department', False):
        return 'department'
    elif getattr(config, 'position', False):
...
def _get_next_step_after_residence_status(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after residence status collection.
    """
    config = bot_instance.config.application_settings.data_collection_fields
    
    if getattr(config, 'governorate', False):
        return 'governorate'
    else:
        return 'complaint_type'
...
def _get_next_step_after_governorate(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after governorate collection.
    """
    config = bot_instance.config.application_settings.data_collection_fields
    
    if getattr(config, 'directorate', False):
        return 'directorate'
    else:
        return 'complaint_type'
...
def _get_next_step_after_directorate(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after directorate collection.
    """
    config = bot_instance.config.application_settings.data_collection_fields
    
    if getattr(config, 'village', False):
        return 'village'
    elif getattr(config, 'department', False):
        return 'department'
    elif getattr(config, 'position', False):
...
def _get_next_step_after_village(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after village collection.
    """
    config = bot_instance.config.application_settings.data_collection_fields
...
def _get_next_step_after_department(bot_instance: InstitutionBot) -> str:
    """
    Determines the next step in the data collection flow after department collection.
    """
    config = bot_instance.config.application_settings.data_collection_fields
...
async def process_sex(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        valid_options = [
            get_message('btn_male', bot_instance, is_arabic),
            get_message('btn_female', bot_instance, is_arabic),
            get_message('btn_prefer_not_say', bot_instance, is_arabic),
            get_message('btn_skip', bot_instance, is_arabic)
        ]
        
        if sex_input not in valid_options:
            await update.message.reply_text(get_message('error_invalid_selection', bot_instance, is_arabic))
            return await collect_sex(update, context, is_arabic)
        
        complaint_data.sex = "" if sex_input == get_message('btn_skip', bot_instance, is_arabic) else sex_input
...
async def process_residence_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Processes the user's residence status input and transitions to next appropriate step."""
    bot_instance: InstitutionBot = context.bot_data['bot_instance']
    user = update.effective_user
    is_arabic = get_user_preferred_language_is_arabic(update, bot_instance)
    complaint_data = _get_complaint_data(user.id, context)
    ...
    try:
        ...
        if not residence_input:
            await update.message.reply_text(get_message('validation_error_residence', bot_instance, is_arabic))
            return states.COLLECTING_RESIDENCE
...
async def process_governorate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Processes the user's governorate input and transitions to next appropriate step."""
    bot_instance: InstitutionBot = context.bot_data['bot_instance']
    user = update.effective_user
    is_arabic = get_user_preferred_language_is_arabic(update, bot_instance)
    complaint_data = _get_complaint_data(user.id, context)
...
async def handle_submission_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        if action == "confirm":
            ...
            if not complaint_id:
                await query.edit_message_text(get_message('error_submission_failed', bot_instance, is_arabic))
                return ConversationHandler.END
...
async def handle_critical_submission_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        if action == "confirm":
            ...
            complaint_id = await bot_instance._log_complaint(complaint_data)
            if not complaint_id:
                await query.edit_message_text(get_message('error_submission_failed_critical', bot_instance, is_arabic))
                return ConversationHandler.END
