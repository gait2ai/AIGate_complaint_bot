...
async def _transition_to_next_step(
    ...
    """
    logger.info(f"TRANSITION_HELPER: Attempting to transition from step '{current_step}' for user {update.effective_user.id}")
...
logger.info(f"TRANSITION_HELPER: Determined next step is '{next_step}' for user {update.effective_user.id}")
