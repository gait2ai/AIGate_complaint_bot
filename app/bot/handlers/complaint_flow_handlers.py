...
async def _transition_to_next_step(
    ...
    try:
        ...
        step_to_function = {
            ...
            'complaint_type': collect_complaint_type,
...
async def ask_new_or_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    """
    if 'complaint_flow_started' not in context.user_data:
        # إذا كانت هذه هي المرة الأولى التي ندخل فيها تدفق الشكوى،
        # قم بمسح البيانات لضمان بداية نظيفة.
        
        # نحتفظ ببيانات الشكوى التي تم جمعها من الذكاء الاصطناعي (إذا وجدت)
        existing_complaint_data = context.user_data.get('complaint_data')
        context.user_data.clear()
        if existing_complaint_data:
            context.user_data['complaint_data'] = existing_complaint_data
        
        # نضع علامة لتجنب المسح مرة أخرى في نفس المحادثة
        context.user_data['complaint_flow_started'] = True
        logger.info(f"Complaint flow started for user {update.effective_user.id}. user_data cleared.")
...
async def process_department(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Proceed to next step using centralized transition logic
        return await _transition_to_next_step(update, context, 'department', is_arabic)
...
async def process_position(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Proceed to next step using centralized transition logic
        return await _transition_to_next_step(update, context, 'position', is_arabic)
...
async def process_complaint_type(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        # Proceed to next step using centralized transition logic
        return await _transition_to_next_step(update, context, 'complaint_type', is_arabic)
