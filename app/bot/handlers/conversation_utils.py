...
def is_conversation_active(context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    """
    active = context.user_data.get('conversation_active', False)
    logger.debug(f"Checking conversation active status: {active}")
    return active
...
def set_conversation_state(context: ContextTypes.DEFAULT_TYPE, state: int) -> None:
    ...
    logger.info(f"Set conversation state to: {state}")
...
def get_conversation_state(context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
    ...
    logger.info(f"Retrieved conversation state: {state}")
    return state
...
def clear_conversation_state(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    or when ending the conversation. Enhanced with improved debugging
    ...
    if context.user_data and 'conversation_state' in context.user_data:
        ...
        logger.debug("No conversation state to clear")