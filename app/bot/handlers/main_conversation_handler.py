...
- State-aware text routing for active conversations
"""
...
def set_conversation_state(context: ContextTypes.DEFAULT_TYPE, state: int):
    """
...
def clear_conversation_state(context: ContextTypes.DEFAULT_TYPE):
    ...
    or when ending the conversation.
    """
...
async def handle_initial_text_message(
    ...
    2. For active conversations: Route text messages to the current state handler
       by returning the current conversation state.
    
    The function now includes state-aware routing to ensure that text messages
    ...
    try:
        ...
        if current_state is not None:
            logger.info(f"User {update.effective_user.id} in active conversation - routing to state {current_state}")
            # Return the current state to let the ConversationHandler route to the appropriate handler
...
def get_main_conversation_handler():
    ...
    - State-aware routing to ensure text messages reach correct handlers
