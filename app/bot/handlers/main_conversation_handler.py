...
- Prioritizes state-specific handlers over global entry-point handlers.
"""
...
def get_main_conversation_handler():
    ...
    The handler structure prioritizes state-specific handlers over global handlers
    to prevent conversation stalls and state conflicts.
    ...
    return ConversationHandler(
        # Entry points - only explicit command handlers that initiate brand-new conversations
        entry_points=[
            # Primary entry point via /start command
            # This is the only formal entry point to ensure clear conversation initiation
        ...
        # Fallback handlers - actions that can be triggered when no state handlers match
        # or when no conversation is active
        fallbacks=[
            # Handle initial text messages that start a conversation
            # This will only be triggered if no active state handlers match the message
            # When no conversation is active, this acts as a secondary entry point
