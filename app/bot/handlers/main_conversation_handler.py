    ...
    try:
        ...
        if current_state is not None:
            ...
            state_handlers = {
                ...
                COLLECTING_AGE: process_age,
                COLLECTING_NATIONALITY: process_nationality,
        ...
        signal, response_text, is_critical = await bot_instance.analyze_first_contact_message(
        ...
        if signal == "COMPLAINT_START":
            ...
            # Initialize basic complaint data
            complaint_data = ComplaintData(user_id=user.id)
            
            # --- THIS IS THE CORE LOGIC WE ARE ADDING ---
            # Set the is_critical flag from the analysis
            complaint_data.is_critical = is_critical
            # If the complaint is critical, save the initial text immediately
            if is_critical:
                complaint_data.original_complaint_text = message_text
                logger.info(f"Critical complaint detected for user {user.id}. Initial text captured.")
...
def get_main_conversation_handler():
    ...
    return ConversationHandler(
        ...
        states={
            ...
            COLLECTING_NATIONALITY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_nationality)
            ],
