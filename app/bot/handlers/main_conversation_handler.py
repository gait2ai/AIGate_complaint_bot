...
def get_main_conversation_handler():
    ...
    - Initial user entry and action selection
    ...
    return ConversationHandler(
        # Entry points - handlers that initiate brand-new conversations
        entry_points=[
            # Primary entry point via /start command
            CommandHandler('start', start_command),
            # Secondary entry point for direct text messages (non-commands)
        ...
        states={
            ...
            COLLECTING_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_name, per_message=True)
            ],
            ...
            COLLECTING_PHONE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_phone, per_message=True)
            ],
            
            COLLECTING_EMAIL: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_email, per_message=True)
            ],
            ...
            COLLECTING_GOVERNORATE_OTHER: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_governorate_other, per_message=True)
            ],
            
            COLLECTING_DIRECTORATE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_directorate, per_message=True)
            ],
            
            COLLECTING_VILLAGE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_village, per_message=True)
            ],
            
            COLLECTING_DEPARTMENT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_department, per_message=True)
            ],
            
            COLLECTING_POSITION: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_position, per_message=True)
            ],
            
            COLLECTING_COMPLAINT_TYPE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_complaint_type, per_message=True)
            ],
            
            # ===== REGULAR COMPLAINT TEXT STATES =====
            COLLECTING_COMPLAINT_TEXT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_complaint_text, per_message=True)
            ],
            ...
            CRITICAL_COLLECTING_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_critical_name, per_message=True)
            ],
            
            CRITICAL_COLLECTING_PHONE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_critical_phone, per_message=True)
            ],
            
            CRITICAL_COLLECTING_COMPLAINT_TYPE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_critical_complaint_type, per_message=True)
            ],
            
            # ===== CRITICAL COMPLAINT TEXT STATES =====
            CRITICAL_COLLECTING_COMPLAINT_TEXT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_critical_complaint_text, per_message=True)
            ],
            ...
            COLLECTING_SUGGESTION_TEXT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_suggestion_text, per_message=True)
            ],
