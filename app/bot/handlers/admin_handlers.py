...
def format_statistics_message(stats: Dict[str, Any], bot_instance: Any, is_arabic: bool) -> str:
    ...
    try:
        ...
        message_parts = [
            ...
            get_message('admin_stats_total', bot_instance, is_arabic, count=total),
            get_message('admin_stats_critical', bot_instance, is_arabic, 
                       count=critical, percentage=f"{critical_percentage:.1f}")
        ]
        ...
        if status_counts:
            ...
            for status, count in sorted(status_counts.items()):
                ...
                status_display = escape_markdown_v2(
                    get_message(f'status_{status.lower()}', bot_instance, is_arabic)
                )
                message_parts.append(
                    get_message('admin_stats_item', bot_instance, is_arabic,
                              status=status_display, count=count, percentage=f"{percentage:.1f}")
                )
        else:
        ...
        message_parts.append(
            get_message('admin_stats_timestamp', bot_instance, is_arabic, timestamp=timestamp)
        )
