...
def format_statistics_message(stats: Dict[str, Any], bot_instance: Any, is_arabic: bool) -> str:
    ...
    try:
        ...
        # Don't escape individual values, let get_message handle the formatting
        message_parts = [
            get_message('admin_stats_header', bot_instance, is_arabic),
            get_message('admin_stats_total', bot_instance, is_arabic, count=str(total)),
        ...
        # Join all parts and then escape the entire message
        full_message = "\n".join(message_parts)
        return escape_markdown_v2_safe(full_message)
