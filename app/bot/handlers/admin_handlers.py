...
async def export_data(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Display data export options menu to the administrator.
    ...
    Returns:
        int: ADMIN_EXPORT_DATA state to handle export selections
    """
    query = update.callback_query
    user_id = query.from_user.id
    bot_instance = context.bot_data.get('bot_instance')
    ...
    logger.info(f"Data export menu accessed by admin user {user_id}")
    
    # Create export options keyboard
    keyboard = [
        [InlineKeyboardButton(
            get_message('admin_export_complaints', bot_instance, is_arabic),
            callback_data="do_export:complaints"
        )],
        [InlineKeyboardButton(
            get_message('admin_export_beneficiaries', bot_instance, is_arabic),
            callback_data="do_export:beneficiaries"
        )],
        [InlineKeyboardButton(
            get_message('admin_export_notes', bot_instance, is_arabic),
            callback_data="do_export:notes"
        )],
        [InlineKeyboardButton(
            get_message('btn_back_to_admin', bot_instance, is_arabic),
            callback_data="admin_back"
    ...
    await query.edit_message_text(
        get_message('admin_export_menu_prompt', bot_instance, is_arabic),
        reply_markup=reply_markup,
        parse_mode='MarkdownV2'
    )
...
async def handle_export_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """
    Handle the user's export choice and generate the requested CSV file.
    ...
    Returns:
        int: ADMIN_MENU state after processing
    """
    query = update.callback_query
    await query.answer()
    ...
    # Extract data type from callback data (e.g., 'complaints' from "do_export:complaints")
    try:
        data_type = query.data.split(':')[1]
        logger.info(f"Export requested for '{data_type}' by admin user {user_id}")
        
        # Show loading message
        await query.edit_message_text(
            get_message('admin_export_generating', bot_instance, is_arabic, data_type=data_type)
        )
        
        # Call the generic export method
        export_result = await bot_instance.export_data_to_csv(data_type)
        
        if export_result and len(export_result) == 2:
            filename, file_bytes = export_result
            ...
            if file_bytes:
                ...
                await context.bot.send_document(
                    ...
                    document=file_bytes,
                    filename=filename,
                    caption=get_message('admin_export_success', bot_instance, is_arabic, 
                                      data_type=data_type, filename=filename)
                )
                
                # Edit the original message to confirm success
                await query.edit_message_text(
                    get_message('admin_export_complete', bot_instance, is_arabic, 
                              data_type=data_type, filename=filename)
                )
                
                logger.info(f"Export file '{filename}' sent to admin user {user_id}")
            else:
                # No data available
                await query.edit_message_text(
                    get_message('admin_export_no_data', bot_instance, is_arabic, data_type=data_type)
                )
                logger.info(f"No data available for export '{data_type}' requested by user {user_id}")
        else:
            # Export failed or returned unexpected format
            await query.edit_message_text(
                get_message('admin_export_failed', bot_instance, is_arabic, data_type=data_type)
            )
            logger.error(f"Export failed for '{data_type}' requested by user {user_id}")
            
    except Exception as e:
        logger.error(f"Error handling export choice for user {user_id}: {e}")
        await query.edit_message_text(
            get_message('admin_export_error', bot_instance, is_arabic)
        )
    
    # Return to admin menu after processing
...
def get_admin_conversation_handler() -> ConversationHandler:
    ...
    return ConversationHandler(
        ...
        states={
            ...
            ADMIN_EXPORT_DATA: [
                CallbackQueryHandler(handle_export_choice, pattern="^do_export:"),
                CallbackQueryHandler(admin_back, pattern="^admin_back$")
            ]
        },
        fallbacks=[
