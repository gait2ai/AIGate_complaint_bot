...
def format_statistics_message(stats: Dict[str, Any], bot_instance: Any, is_arabic: bool) -> str:
    ...
    try:
        ...
        # Calculate percentages and ensure all values are properly escaped
        critical_percentage = (critical / total * 100) if total > 0 else 0
        
        # Escape all numerical values
        total_escaped = escape_markdown_v2(str(total))
        critical_escaped = escape_markdown_v2(str(critical))
        critical_percentage_escaped = escape_markdown_v2(f"{critical_percentage:.1f}")
        ...
        if status_counts:
            ...
            for status, count in sorted(status_counts.items()):
                ...
                count_escaped = escape_markdown_v2(str(count))
                percentage_escaped = escape_markdown_v2(f"{percentage:.1f}")
                ...
                status_display = escape_markdown_v2(status_message)
